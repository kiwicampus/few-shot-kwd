...
def to_micro_spectrogram(model_settings, audio):
    """
    Convert the audio to a spectrogram using the microfrontend op.
    ...
    Returns:
        TensorFlow spectrogram of the audio.
    """
...
def standard_microspeech_model_settings(label_count: int):
    """
    Prepare the model settings for the standard microspeech model.

    Args:
        label_count (int): The number of classes to be recognized.
...
def add_background(foreground_audio, background_audio, background_volume):
    """
    Mixes background noise into foreground audio.
    """
...
class SpecAugParams:
    """
    Parameters for spectrogram augmentation.
    """
...
class AudioDataset:
    """
    A class used to represent an Audio Dataset for keyword detection.
    """
    ...
    def __init__(
        ...
        """
        Initializes dataset from audio files and properties.
        """
    ...
    def timeshift_samples(self, time_shift_ms=100):
        """
        Calculates allowed time shift range in samples.
        """
    ...
    def random_background_sample(self, background_volume=1.0):
        """
        Selects a random background sample and scales it by the provided volume.

        Args:
            background_volume (float, optional): The volume to scale the background sample. Defaults to 1.0.

        Returns:
            tf.Tensor: A tensor representing the scaled background sample.
        """
    ...
    def get_unknown(self):
        """
        Retrieves an unknown file path randomly from the list of unknown files.

        Returns:
            tf.Tensor: A tensor representing the file path of an unknown file.
        """
    ...
    def augment(self, audio, label):
        """
        Applies random time/frequency masking and noise.
    ...
    def spec_augment(self, spectrogram):
        """
        Implements spectrogram time/frequency masking.

        Args:
            spectrogram: Tensor of spectrogram data.

        Returns:
            Tensor of spectrogram data.
        """
        # https://git.io/JLvGB
    ...
    def map_spec_aug(self, spectrogram, label_id):
        """
        Applies spec augment with given probability.

        Args:
            spectrogram: Tensor of spectrogram data.
            label_id: Tensor of label data.

        Returns:
            A tuple of the spectrogram and label data.
        """
    ...
    def get_background_data(self, background_dir):
        """
        Reads background noise samples into memory.
        """
    ...
    def decode_audio(self, audio_binary):
        """
        Decodes the audio from the given audio binary.

        Args:
            audio_binary: Binary data representing the audio.
    ...
    def get_label(self, file_path):
        """
        Retrieves the label from the given file path.

        Args:
            file_path (str): The path of the file.

        Returns:
            str: The label extracted from the file path.
        """
    ...
    def get_waveform_and_label(self, file_path):
        """
        Retrieves the waveform and label from the given file path.

        Args:
            file_path (str): The path of the file.

        Returns:
            tuple: A tuple containing the waveform and label.
        """
    ...
    def get_spectrogram_and_label_id(self, audio, label):
        """
        Retrieves the spectrogram and label ID from the given audio and label.
    ...
    def get_label_id_from_filename(self, filepath):
        """
        Retrieves the label ID from the given file path.

        Args:
            filepath (str): The path of the file.

        Returns:
            int: The label ID extracted from the file path.
        """
        # only for rebalancer
    ...
    def file2spec_w_bg(self, filepath):
        """
        Reads an audio file, decodes it, adds background noise, and converts it to a spectrogram.

        Args:
            filepath: Path to the audio file.

        Returns:
            A spectrogram of the audio with added background noise.
        """
        audio_binary = tf.io.read_file(filepath)
        waveform = self.decode_audio(audio_binary)
