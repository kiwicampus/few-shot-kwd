...
iso2counts_train_sorted = sorted(
    list(iso2count_utts_train.items()), key=lambda kv: kv[1], reverse=True
)
for ix, (k, v) in enumerate(iso2counts_train_sorted):
    print(k, v)
    df.loc[ix] = [
        iso2lang[k],
        len(iso2count_words_val[k]),
        v,
        iso2count_utts_val[k],
        iso2valacc[k] * 100,
    ]
