...
for word_dir in clips_dir.glob("*"):
    if not word_dir.name in commands:
        ...
        csv_path = (
            "/mnt/disks/std750/colby/distances/distances_k_"
            + str(k_clust)
            + "/"
            + word_dir.name
            + "_distances.csv"
...
cv_model = train(
    ...
    cv_other_train_files,
    model_settings,
    "msc_filter_p" + str(percentile_cutoff) + "_k" + str(k_clust) + "_n" + str(n_clust),
)

cv_model = tf.keras.models.load_model(
    "/mnt/disks/std750/colby/multilingual_kws/checkpoints/dscnn_msc_filter_p"
    + str(percentile_cutoff)
    + "_k"
    + str(k_clust)
    + "_n"
    + str(n_clust)
